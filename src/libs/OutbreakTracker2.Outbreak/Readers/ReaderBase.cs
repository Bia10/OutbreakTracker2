...
public abstract class ReaderBase : IDisposable
{
        ...
        where T : unmanaged
    {
        ...
        {
            Logger.LogWarning(
                "[{MethodName}] Attempted to read {Type} from a null address.",
                methodName,
                typeof(T).Name
            );
            return default;
        ...
        catch (AccessViolationException ex)
        {
            Logger.LogError(
                ex,
                "[{MethodName}] Access violation reading {Type} from address 0x{Address:X}",
                methodName,
                typeof(T).Name,
                address
            );
        ...
        catch (Exception ex)
        {
            Logger.LogError(
                ex,
                "[{MethodName}] Unexpected error reading {Type} from address 0x{Address:X}",
                methodName,
                typeof(T).Name,
                address
            );
    ...
    )
    {
        ...
        {
            Logger.LogWarning(
                "[{MethodName}] Computed address is null (0x{ComputedAddress:X}) from base pointer 0x{BasePointer:X} with {OffsetCount} offsets. This may indicate an issue in the pointer chain.",
                methodName,
                computedAddress,
                basePtr,
                offsets.Length
    ...
    private nint ComputeAddress(ReadOnlySpan<nint> offsets, [CallerMemberName] string methodName = "")
    {
        ...
        {
            Logger.LogWarning(
                "[{MethodName}] Computed address is null (0x{ComputedAddress:X}) from {OffsetCount} offsets (first offset: 0x{FirstOffset:X}). This may indicate an issue in the pointer chain.",
                methodName,
                computedAddress,
                offsets.Length,
                offsets[0]
    ...
    )
    {
        ...
        {
            Logger.LogTrace(
                "[{MethodName}] Selected offsets based on current game file {GameFile}",
                methodName,
                CurrentFile
            );
        }
    ...
    private nint GetLobbyBasePointer(int slotIndex, [CallerMemberName] string methodName = "")
    {
        if (!slotIndex.IsSlotIndexValid())
        {
            Logger.LogWarning(
                "[{MethodName}] Invalid slot index provided: {SlotIndex}. Expected range: 0 to {MaxLobbySlots}",
                methodName,
                slotIndex,
                GameConstants.MaxLobbySlots - 1
            );
            return nint.Zero;
        }
    ...
    )
    {
        ...
        Logger.LogTrace(
            "[{MethodName}] Successfully computed lobby address 0x{Address:X} for slot {SlotIndex} using base pointer 0x{BasePointer:X}",
            methodName,
            address,
            slotIndex,
            basePtr
    ...
    )
    {
        ...
        {
            Logger.LogTrace(
                "[{MethodName}] Successfully computed address 0x{Address:X} using offsets for current game file {GameFile}",
                methodName,
                address,
                CurrentFile
            );
        ...
        errorMessage =
            $"[{methodName}] Computed address 0x{address:X} is invalid (null or negative) using provided offsets for current game file {CurrentFile}.";
        return false;
    }
    ...
    )
    {
        ...
        {
            Logger.LogWarning(
                "[{MethodName}] Read operation for {Type} failed because address was null (0x{Address:X}).",
                methodName,
                typeof(TResult).Name,
                address
            );
    ...
    protected T ReadValue<T>(
        ...
        where T : unmanaged =>
        ReadValueFromOffsetsInternal(
            offsetsFile1,
            offsetsFile2,
            errorValue,
            readOperation: address => Read<T>(address, methodName),
            methodName
        );
    ...
    )
    {
        ...
        {
            Logger.LogWarning(
                "[{MethodName}] Read operation for {Type} failed for slot {SlotIndex} because address was null (0x{Address:X}).",
                methodName,
                typeof(TResult).Name,
                slotIndex,
                address
        ...
        where T : unmanaged
    {
        ...
        {
            Logger.LogDebug(
                "[{MethodName}] Read value of type {Type} at address 0x{Address:X} returned default value. This might indicate a read error.",
                methodName,
                typeof(T).Name,
                address
            );
        ...
        where TFileTwo : struct, Enum
    {
        ...
        Logger.LogDebug(
            "[{MethodName}] Retrieved scenario string '{ScenarioName}' for ID {ScenarioId} with game file {GameFile}",
            nameof(GetScenarioString),
            scenarioName,
            scenarioId,
            CurrentFile
