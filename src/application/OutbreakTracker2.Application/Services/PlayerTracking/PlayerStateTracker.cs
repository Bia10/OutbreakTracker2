...
public sealed class PlayerStateTracker : IPlayerStateTracker
{
    ...
    public PlayerStateTracker()
    {
        ...
        PlayerStateChanges = _playerUpdateSubject.SelectMany(currentPlayer =>
        {
        ...
        IReadOnlyList<PlayerStateTrackingRule> playerTrackingRules = new PlayerStateTrackerBuilder()
            ...
            .TrackGeneralChange(
                (current, last) =>
                    current.CurHealth <= 0
                    && last.CurHealth > 0
                    && !string.Equals(current.Status, "Zombie", StringComparison.Ordinal),
                current => new PlayerStateChangeEventArgs(
                    $"{current.Name} health dropped to 0!",
                    "Player Died",
                    ToastType.Error
                )
            )
