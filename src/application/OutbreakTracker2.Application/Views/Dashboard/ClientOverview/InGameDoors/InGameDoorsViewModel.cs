...
public class InGameDoorsViewModel : ObservableObject, IDisposable
{
    ...
    )
    {
        ...
        _subscription = dataManager
            ...
            .SubscribeAwait(
                async (incomingDoorsSnapshot, ct) =>
                {
                    switch (incomingDoorsSnapshot.Length)
                    {
                        ...
                        case > GameConstants.MaxDoors:
                            _logger.LogWarning(
                                "Received more doors then is the inGame limit. Entries {Length}",
                                incomingDoorsSnapshot.Length
                            );
                            return;
                        default:
                            _logger.LogInformation(
                                "Processing doors snapshot on thread pool with {Length} entries",
                                incomingDoorsSnapshot.Length
                            );
                            try
                            {
                                ...
                                _logger.LogInformation(
                                    "Processed {Count} filtered door entries on thread pool",
                                ...
                                foreach (DecodedDoor incomingDoor in filteredIncomingDoors.AsValueEnumerable())
                                {
                                    ...
                                    if (
                                        _viewModelCache.TryGetValue(
                                            incomingDoor.Id,
                                            out InGameDoorViewModel? existingVm
                                        )
                                    )
                                    {
                                ...
                                await _dispatcherService
                                    .InvokeOnUIAsync(
                                        () =>
                                        {
                                            ...
                                            {
                                                if (
                                                    incomingDoorDataMap.TryGetValue(
                                                        vm.UniqueId,
                                                        out DecodedDoor? doorData
                                                    )
                                                )
                                                {
                                                    vm.Update(doorData);
                                                    ...
                                                    _logger.LogTrace(
                                                        "Updating ViewModel properties and adding to cache on UI thread for {UniqueId}",
                                                        vm.UniqueId
                                                    );
                                                }
                                                else
                                                {
                                                    _logger.LogWarning(
                                                        "Door data not found in map for VM {UniqueId} during UI update. This should not happen",
                                                        vm.UniqueId
                                                    );
                                                }
                                            }
                                            ...
                                            for (int i = Doors.Count - 1; i >= 0; i--)
                                            {
                                                ...
                                                _logger.LogDebug(
                                                    "Removing ViewModel from Doors list & Cache on UI thread for UniqueId: {UniqueId}",
                                            ...
                                            if (Doors.Count != desiredViewModels.Count)
                                                _logger.LogWarning(
                                                    "_doors count ({DCount}) differs from desiredViewModels count ({DesCount}) after sync. Adjusting",
                                                    Doors.Count,
                                                    desiredViewModels.Count
                                                );
