...
public sealed class LobbySlotsViewModel : ObservableObject, IAsyncDisposable
{
    ...
    )
    {
        ...
        _subscription = dataManager
            .LobbySlotsObservable.ObserveOnThreadPool()
            .SubscribeAwait(
                async (lobbySlotsSnapshot, cancellationToken) =>
                {
                    _logger.LogInformation(
                        "Processing lobby slots snapshot on thread pool with {Length} entries",
                        ...
                        for (int i = 0; i < lobbySlotsSnapshot.Length; i++)
                        {
                            ...
                            if (i < _lobbySlotsInternal.Count)
                            {
                                ...
                                else
                                {
                                    ...
                                    _logger.LogWarning(
                                        "VM {Ulid} from old list snapshot at index {Index} not found in cache or its ULID changed. Creating new VM for incoming data",
                                        currentVmInList.Id,
                                        i
                                    );
                                }
                                ...
                                () =>
                                {
                                    ...
                                    foreach (Ulid ulid in ulidsToRemoveFromCache)
                                    {
                                        if (_viewModelCache.Remove(ulid))
                                            _logger.LogDebug(
                                                "Removing VM from cache on UI thread for ULID: {Ulid}",
                                                ulid
                                            );
                                        else
                                            _logger.LogWarning(
                                                "Attempted to remove VM {Ulid} from cache but it was not found",
                                                ulid
                                            );
                                    }
                                    ...
                                    foreach (LobbySlotViewModel vm in desiredViewModelsForUiList)
                                    {
                                        if (!_viewModelCache.ContainsKey(vm.Id))
                                        {
                                            ...
                                            _logger.LogTrace(
                                                "Adding new VM to cache on UI thread for ULID: {Ulid}",
                                                vm.Id
                                            );
                                        }
                                    }
                                    ...
                                    for (int i = _lobbySlotsInternal.Count - 1; i >= 0; i--)
                                    {
                                        ...
                                        {
                                            _logger.LogDebug(
                                                "Removing VM from UI list: ULID {Ulid} at index {Index} (no longer active)",
                                                currentVmInList.Id,
                                                i
                                            );
                                    ...
                                    for (int i = 0; i < desiredViewModelsForUiList.Count; i++)
                                    {
                                        ...
                                        {
                                            _logger.LogDebug(
                                                "Inserting VM into UI list: ULID {Ulid} at index {Index}",
                                                desiredVm.Id,
                                                i
                                            );
                                        ...
                                        {
                                            _logger.LogDebug(
                                                "Moving VM in UI list: ULID {Ulid} from index {FromIndex} to index {ToIndex}",
                                                desiredVm.Id,
                                                currentIndexInList,
                                                i
                                            );
                                    ...
                                    _logger.LogTrace(
                                        "UI list count after adds/moves/removals: {Count}",
                                    ...
                                    _logger.LogInformation(
                                        "UI update complete. LobbySlots UI list count: {Count}",
                                        _lobbySlotsInternal.Count
                                    );
                                },
                                cancellationToken
