...
public sealed class HtmlRunReportWriterTests
{
    ...
    [Test]
    public async Task WriteAsync_ContentContainsPreciseInventorySlotValues()
    {
        (HtmlRunReportWriter writer, string outputDirectory, IDisposable cleanup) = CreateWriterWithTempDir();
        using (cleanup)
        {
            ...
            await Assert.That(content.Contains("Old Slot Value", StringComparison.Ordinal)).IsTrue();
            await Assert.That(content.Contains("0x00 | 0", StringComparison.Ordinal)).IsTrue();
            await Assert.That(content.Contains("0x21 | 33", StringComparison.Ordinal)).IsTrue();
        }
    }
