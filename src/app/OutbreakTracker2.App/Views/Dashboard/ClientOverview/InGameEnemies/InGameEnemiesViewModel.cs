...
public class InGameEnemiesViewModel : ObservableObject, IDisposable
{
        ...
        IDataManager dataManager)
    {
        ...
        _subscription = dataManager.EnemiesObservable
            ...
            .SubscribeAwait(async (incomingEnemiesSnapshot, ct) =>
            {
                ...
                {
                    case 0:
                        _logger.LogWarning("Received empty or null enemies snapshot. Entries {Length}", incomingEnemiesSnapshot.Length);
                        return;
                    case > GameConstants.MaxEnemies2:
                        _logger.LogWarning("Received more enemies than the in-game limit. Entries {Length}", incomingEnemiesSnapshot.Length);
                        return;
                    default:
                        _logger.LogInformation("Processing enemies snapshot on thread pool with {Length} entries", incomingEnemiesSnapshot.Length);
                        try
                        {
                            ...
                            await _dispatcherService.InvokeOnUIAsync(() =>
                            {
                                ...
                                for (int i = 0; i < desiredViewModels.Count; i++)
                                {
                                    ...
                                    else if (currentIndexInEnemies != i)
                                    {
                                        _logger.LogDebug("Moving ViewModel in ObservableList on UI thread: {UniqueId} from index {FromIndex} to index {ToIndex}",
                                            desiredVm.UniqueId, currentIndexInEnemies, i);
                                ...
                                {
                                    _logger.LogWarning("_enemies count ({ECount}) differs from desiredViewModels count ({DesCount}) after sync. This indicates an issue in logic",
                                        _enemies.Count, desiredViewModels.Count);
                                }
    ...
    private static bool IsEnemyActive(DecodedEnemy enemy)
        => !string.IsNullOrEmpty(enemy.Name) && !enemy.RoomName.Equals("Spawning/Scenario Cleared", StringComparison.Ordinal)
